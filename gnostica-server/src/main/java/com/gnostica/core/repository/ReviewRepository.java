...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(AVG(r.rating), 0.0) FROM Review r WHERE r.course.account.email = :instructorEmail AND r.course.deletedAt IS NULL AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(AVG(r.rating), 0.0) FROM Review r WHERE r.course.account.email = :instructorEmail AND r.course.deletedAt IS NULL AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL AND r.createdAt >= :startDate AND r.createdAt < :endDate")
    Double getAverageRatingByInstructorEmailAndDateRange(
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(r) FROM Review r WHERE r.course.account.email = :instructorEmail AND r.course.deletedAt IS NULL AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL")
    long countValidRatingsByInstructorEmail(@org.springframework.data.repository.query.Param("instructorEmail") String instructorEmail);

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(AVG(r.rating), 0.0) FROM Review r WHERE r.course.id = :courseId AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL")
    Double getAverageRatingByCourseId(@org.springframework.data.repository.query.Param("courseId") java.util.UUID courseId);

    @org.springframework.data.jpa.repository.Query("SELECT r.rating, COUNT(r) FROM Review r WHERE r.course.account.email = :instructorEmail AND r.course.deletedAt IS NULL AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL GROUP BY r.rating")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT r FROM Review r WHERE r.course.account.email = :instructorEmail AND r.course.deletedAt IS NULL AND r.deletedAt IS NULL AND r.status = 1 AND r.parent IS NULL ORDER BY r.createdAt DESC")
