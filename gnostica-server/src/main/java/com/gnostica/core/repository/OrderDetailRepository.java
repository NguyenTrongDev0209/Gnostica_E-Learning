...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, java.util.UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)), 0.0) FROM OrderDetail od WHERE od.course.account.email = :instructorEmail AND od.order.status = 1 AND od.status = 1 AND od.order.createdAt >= :startDate AND od.order.createdAt < :endDate")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)), 0.0) FROM OrderDetail od WHERE od.course.account.email = :instructorEmail AND od.order.status = 1 AND od.status = 1")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)), 0.0) FROM OrderDetail od WHERE od.status = 1 AND od.order.status = 1")
    Double sumTotalCourseRevenue();

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)), 0.0) FROM OrderDetail od WHERE od.status = 1 AND od.order.status = 1 AND od.order.createdAt >= :startDate AND od.order.createdAt < :endDate")
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0)), 0.0) FROM OrderDetail od LEFT JOIN od.commission c WHERE od.status = 1 AND od.order.status = 1")
    Double sumTotalInstructorRevenue();

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(od.order.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0)), 0.0) FROM OrderDetail od LEFT JOIN od.commission c WHERE od.status = 1 AND od.order.status = 1 AND od.order.createdAt >= :startDate AND od.order.createdAt < :endDate")
