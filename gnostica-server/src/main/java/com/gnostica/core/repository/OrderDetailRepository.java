...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, java.util.UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(CASE WHEN (coupon.id IS NOT NULL AND couponAccountRole.name IS NOT NULL AND UPPER(couponAccountRole.name) = 'ADMIN') THEN (od.price * (100 - COALESCE(od.discount, 0)) / 100.0) * (COALESCE(c.instructorRatio, 90.0) / 100.0) ELSE ((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(o.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0) END), 0.0) FROM OrderDetail od JOIN od.order o LEFT JOIN o.coupon coupon LEFT JOIN coupon.account couponAccount LEFT JOIN couponAccount.role couponAccountRole LEFT JOIN od.commission c WHERE od.course.account.email = :instructorEmail AND od.status = 1 AND o.status = 1 AND o.createdAt >= :startDate AND o.createdAt < :endDate")
    Double sumInstructorEarningByEmailAndDateRange(
            @org.springframework.data.repository.query.Param("instructorEmail") String instructorEmail,
            @org.springframework.data.repository.query.Param("startDate") java.time.LocalDateTime startDate,
            @org.springframework.data.repository.query.Param("endDate") java.time.LocalDateTime endDate);

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(CASE WHEN (coupon.id IS NOT NULL AND couponAccountRole.name IS NOT NULL AND UPPER(couponAccountRole.name) = 'ADMIN') THEN (od.price * (100 - COALESCE(od.discount, 0)) / 100.0) * (COALESCE(c.instructorRatio, 90.0) / 100.0) ELSE ((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(o.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0) END), 0.0) FROM OrderDetail od JOIN od.order o LEFT JOIN o.coupon coupon LEFT JOIN coupon.account couponAccount LEFT JOIN couponAccount.role couponAccountRole LEFT JOIN od.commission c WHERE od.course.account.email = :instructorEmail AND od.status = 1 AND o.status = 1")
    Double sumTotalInstructorEarningByEmail(
            @org.springframework.data.repository.query.Param("instructorEmail") String instructorEmail);
    ...
    /** Thu nhập RÒNG (sau hoa hồng, theo commission snapshot) của giảng viên trong khoảng thời gian. */
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(CASE WHEN (coupon.id IS NOT NULL AND couponAccountRole.name IS NOT NULL AND UPPER(couponAccountRole.name) = 'ADMIN') THEN (od.price * (100 - COALESCE(od.discount, 0)) / 100.0) * (COALESCE(c.instructorRatio, 90.0) / 100.0) ELSE ((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(o.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0) END), 0.0) FROM OrderDetail od JOIN od.order o LEFT JOIN o.coupon coupon LEFT JOIN coupon.account couponAccount LEFT JOIN couponAccount.role couponAccountRole LEFT JOIN od.commission c WHERE od.course.account = :account AND od.status = 1 AND o.status = 1 AND o.createdAt >= :startDate AND o.createdAt < :endDate")
    Double sumInstructorEarningByAccountAndDateRange(
            @org.springframework.data.repository.query.Param("account") com.gnostica.core.model.Account account,
            @org.springframework.data.repository.query.Param("startDate") java.time.LocalDateTime startDate,
            @org.springframework.data.repository.query.Param("endDate") java.time.LocalDateTime endDate);

    /** Thu nhập RÒNG (sau hoa hồng) tích lũy TOÀN KỲ của giảng viên. */
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(CASE WHEN (coupon.id IS NOT NULL AND couponAccountRole.name IS NOT NULL AND UPPER(couponAccountRole.name) = 'ADMIN') THEN (od.price * (100 - COALESCE(od.discount, 0)) / 100.0) * (COALESCE(c.instructorRatio, 90.0) / 100.0) ELSE ((od.price * (100 - COALESCE(od.discount, 0)) / 100.0) - COALESCE(o.couponPrice, 0.0)) * (COALESCE(c.instructorRatio, 90.0) / 100.0) END), 0.0) FROM OrderDetail od JOIN od.order o LEFT JOIN o.coupon coupon LEFT JOIN coupon.account couponAccount LEFT JOIN couponAccount.role couponAccountRole LEFT JOIN od.commission c WHERE od.course.account = :account AND od.status = 1 AND o.status = 1")
    Double sumTotalInstructorEarningByAccount(@org.springframework.data.repository.query.Param("account") com.gnostica.core.model.Account account);
