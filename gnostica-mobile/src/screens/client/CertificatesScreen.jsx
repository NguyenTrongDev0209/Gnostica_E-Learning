    ...
    return (
        <View className="flex-1 bg-slate-50">
            ...
            <View className="bg-white pt-12 pb-4 px-4 border-b border-slate-100 flex-row items-center">
                ...
                <AppText className="text-xl font-bold text-slate-800 ml-2">Chứng chỉ của tôi</AppText>
            </View>
                ...
                {MOCK_CERTIFICATES.length === 0 ? (
                    ...
                    MOCK_CERTIFICATES.map(item => (
                        <View key={item.id} className="bg-white rounded-3xl overflow-hidden mb-6 shadow-sm border border-slate-100">
                            ...
                            />
                            <View className="p-5">
                                <AppText className="text-slate-900 font-bold text-lg mb-1">{item.courseTitle}</AppText>
                                <View className="flex-row justify-between items-center mb-4">
                                    <AppText className="text-slate-500 text-xs">Ngày cấp: {item.date}</AppText>
                                    <AppText className="text-slate-500 text-xs">Mã: {item.code}</AppText>
                                </View>
