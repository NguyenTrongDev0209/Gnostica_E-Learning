...
const CourseDetailScreen = () => {
    ...
    return (
        <View className="flex-1 bg-slate-50">
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View className="relative">
                    ...
                    {badge && (
                        ...
                        >
                            <AppText className="text-xs font-bold" style={{ color: badge.text }}>
                                {course.badge}
                            </AppText>
                        </View>
                    )}
                ...
                <View className="bg-white p-5 mb-2">
                    <AppText className="text-sm text-blue-600 font-semibold mb-2">{course.category}</AppText>
                    <AppText className="text-[20px] font-extrabold text-slate-800 leading-7 mb-3">
                        {course.title}
                    </AppText>
                    ...
                    <View className="flex-row gap-5 mt-3">
                        ...
                        <View className="flex-row items-center gap-[5px]">
                            ...
                            <AppText className="text-[13px] text-slate-500">{course.duration}</AppText>
                        </View>
                        <View className="flex-row items-center gap-[5px]">
                            <BookOpen size={14} color="#64748B" />
                            <AppText className="text-[13px] text-slate-500">{course.level}</AppText>
                        </View>
                    </View>
                    ...
                    <View className="mt-4 pt-4 border-t border-slate-100 flex-row items-center gap-3">
                        ...
                        <View>
                            <AppText className="text-xs text-slate-400 font-medium">Giảng viên</AppText>
                            <AppText className="text-sm text-slate-800 font-bold">{course.instructor}</AppText>
                        </View>
                    </View>
                </View>
                ...
                {/* Description */}
                <View className="bg-white p-5 mb-2">
                    <AppText className="text-base font-extrabold text-slate-800 mb-2.5">Mô tả khóa học</AppText>
                    <AppText className="text-sm text-slate-600 leading-[22px]">{course.description}</AppText>
                </View>

                {/* Curriculum */}
                <View className="bg-white p-5 mb-2">
                    <AppText className="text-base font-extrabold text-slate-800 mb-3">Nội dung khóa học</AppText>
                    {course.curriculum?.map((section, i) => (
                        ...
                        >
                            <View className="flex-1">
                                <AppText className="text-sm font-semibold text-slate-800">{section.section}</AppText>
                                {expandedSection === i && (
                                    <AppText className="text-[13px] text-slate-500 mt-1">
            ...
            >
                <View className="flex-1">
                    <AppText className="text-lg font-extrabold text-blue-600">{course.price}</AppText>
                    {course.originalPrice && (
                        <AppText className="text-xs text-slate-400 line-through">{course.originalPrice}</AppText>
                    )}
                </View>
                <Button
