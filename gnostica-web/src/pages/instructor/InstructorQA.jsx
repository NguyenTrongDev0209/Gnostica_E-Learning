  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Hỏi Đáp & Đánh Giá</h1>
        ...
        <div className="flex text-sm font-bold text-slate-500 bg-slate-100 p-1 rounded-lg">
          <button className="px-4 py-2 rounded-md bg-white text-slate-900 shadow-sm">Tất cả</button>
          <button className="px-4 py-2 rounded-md hover:text-slate-900 transition-colors">Chưa trả lời</button>
        </div>
      </div>

      <Tabs defaultValue="questions" className="w-full">
        <TabsList className="mb-6 bg-slate-100 p-1">
          <TabsTrigger value="questions" className="px-8 py-2.5 font-bold data-[state=active]:bg-white data-[state=active]:text-green-600 data-[state=active]:shadow-sm">
        ...
        <TabsContent value="questions" className="space-y-4">
           {MOCK_QUESTIONS.map((q) => (
             <Card key={q.id} className="border-slate-200 shadow-sm hover:border-green-200 transition-colors group">
                 ...
                 <div className="p-5 flex gap-4">
                   ...
                   <div className="flex-1 min-w-0">
                     <div className="flex justify-between items-start mb-2">
                       <div>
                         <h4 className="font-bold text-slate-900">{q.user}</h4>
                         <p className="text-xs text-slate-400 font-bold uppercase tracking-tight">{q.time}</p>
                       </div>
                       {q.status === 'unanswered' ? (
                     ...
                     <p className="text-sm font-bold text-slate-700 mb-3 bg-slate-50 p-3 rounded-lg border-l-4 border-l-green-500">
                       "{q.content}"
                     </p>
                     <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-bold text-slate-400 uppercase tracking-tighter">
                       <span className="flex items-center gap-1">Khóa: <span className="text-slate-600 font-black">{q.course}</span></span>
                       <span className="flex items-center gap-1">Bài: <span className="text-slate-600 font-black">{q.lesson}</span></span>
                     </div>
                   </div>
                 </div>
                 <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center group-hover:bg-green-50/30 transition-colors">
                    <div className="flex items-center gap-4">
                      <button className="text-xs font-bold text-slate-400 hover:text-slate-600 flex items-center gap-1.5">
                        <ThumbsUp className="w-3.5 h-3.5" /> Hữu ích
                      </button>
                    </div>
                    <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white font-bold h-8 shadow-none">
        ...
        <TabsContent value="reviews" className="space-y-4">
           {MOCK_REVIEWS.map((r) => (
             <Card key={r.id} className="border-slate-200 shadow-sm hover:border-amber-200 transition-colors">
               <CardContent className="p-0">
                 <div className="p-5 flex gap-4">
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-slate-200 shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                       <div className="flex justify-between items-start mb-2">
                         <div>
                            <h4 className="font-bold text-slate-900">{r.user}</h4>
                         ...
                         <span className="text-xs text-slate-400 font-bold uppercase">{r.time}</span>
                       </div>
                       <p className="text-sm font-medium text-slate-600 py-3 italic">"{r.content}"</p>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 py-0.5 bg-slate-100 w-fit rounded">
                         Khóa học: <span className="text-slate-800">{r.course}</span>
                       </p>
                    </div>
                 </div>
