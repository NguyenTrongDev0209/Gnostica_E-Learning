    ...
    <div className="bg-background min-h-screen pb-24">
      <div className="app-container pt-8 md:pt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 xl:gap-12">
          ...
          {/* =========================================
              LEFT COLUMN: 8/12 Width (Course Material)
              ========================================= */}
          <div className="lg:col-span-8 flex flex-col gap-10">
            ...
            {/* 2. Main Video Player Area (Player-First Focus) */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                {course.outcomes.map((outcome, idx) => (
                  <div key={idx} className="flex items-start gap-4">
                    ...
                    <span className="text-slate-700 text-[15px] leading-relaxed font-medium">{outcome}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <Accordion type="multiple" className="w-full bg-white border border-slate-200 rounded-[20px] overflow-hidden shadow-sm">
                    ...
                    <AccordionContent className="bg-white pb-0">
                      <div className="divide-y divide-slate-100/80 border-t border-slate-100">
                        {section.lessons.map((lesson, lIdx) => (
                          <div key={lIdx} className="flex items-center justify-between px-6 py-4 group hover:bg-slate-50 transition-colors">
                            <div className="flex items-center gap-4">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${lesson.preview ? 'bg-primary/10 text-primary' : 'bg-slate-100 text-slate-400'}`}>
                                <PlayCircle className="w-4 h-4" />
                              </div>
                              <span className={`text-[15px] font-semibold ${lesson.preview ? 'text-slate-900 group-hover:text-primary transition-colors cursor-pointer' : 'text-slate-600'}`}>
                                {lesson.title}
                              </span>
                            </div>
                            <div className="flex items-center gap-4 text-sm text-slate-500 font-medium">
                              {lesson.preview && (
                                <button className="text-primary font-bold hover:underline hidden sm:block">Học thử</button>
                              )}
                ...
                <div className="flex-1 space-y-5">
                  <div>
                    <h3 className="text-2xl font-extrabold text-slate-900 mb-1.5 flex items-center gap-2">
                      ...
                      <span className="bg-primary/10 text-primary text-[10px] uppercase font-black px-2 py-1 rounded-md">Pro Mentor</span>
                    </h3>
                  ...
                  <Button variant="outline" className="mt-2 font-bold border-slate-300 text-slate-700 hover:text-primary hover:border-primary">
          ...
          {/* =========================================
              RIGHT COLUMN: 4/12 Width (Sticky Pricing)
              ========================================= */}
              ...
              <Card className="border-slate-200/80 shadow-2xl shadow-slate-200/50 rounded-[28px] overflow-hidden bg-white">
                ...
                <CardContent className="p-7 md:p-8">
                  <div className="flex items-start justify-between mb-2">
                    <Badge variant="destructive" className="bg-red-100 text-red-600 hover:bg-red-100 border-none font-bold px-3 py-1">
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <Button variant="outline" className="w-full py-7 text-lg font-bold rounded-2xl border-2 border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors">
                      Thêm vào giỏ hàng
                    </Button>
                  </div>
                  ...
                  <div>
                    <h4 className="font-extrabold mb-5 text-slate-900 text-[15px] uppercase tracking-wider">Khóa học này bao gồm:</h4>
