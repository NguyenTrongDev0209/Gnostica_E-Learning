...
const ApplyInstructor = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await instructorService.createApplication({
        ...
        <div className="min-h-screen bg-slate-50/50 py-12 px-4 sm:px-6">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-12 relative">
                    <div className="absolute top-1/2 left-0 w-full h-[2px] bg-slate-200 -translate-y-1/2 z-0 hidden sm:block" />
                                ...
                                {currentStep === 1 && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 border-l-4 border-primary pl-4 py-1">
                                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Hồ sơ cá nhân công khai</h2>
                                        </div>

                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div className="space-y-2.5">
                                                <Label className="text-slate-600 font-bold ml-1">Họ và tên hiển thị</Label>
                                                <div className="relative">
                                                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
                                                    <Input value={currentUser.fullName} disabled className="pl-10 h-12 bg-slate-50 border-slate-200 font-semibold cursor-not-allowed opacity-80" />
                                                </div>
                                            </div>
                                            <div className="space-y-2.5">
                                                <Label className="text-slate-600 font-bold ml-1">Địa chỉ Email</Label>
                                                <div className="relative">
                                                    <CheckCircle2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-emerald-500" />
                                                    <Input title="Email đã được xác thực" value={currentUser.email} disabled className="pl-10 h-12 bg-slate-50 border-slate-200 font-semibold cursor-not-allowed opacity-80" />
                                                </div>
                                            </div>
                                            <div className="space-y-2.5 md:col-span-2">
                                                <Label className="text-slate-600 font-bold ml-1 flex items-center gap-1.5">
                                                    Số điện thoại chính xác <span className="text-rose-500">*</span>
                                                </Label>
                                                <div className="relative">
                                                    <Smartphone className="absolute left-3 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400 group-focus-within:text-primary" />
                                                    <Input
                                                        ...
                                                        className={`pl-10 h-14 text-lg border-2 ring-offset-background focus-visible:ring-0 transition-all ${errors.contactPhone ? 'border-rose-200 bg-rose-50/30' : 'border-slate-100 focus:border-primary/30 focus:bg-white'}`}
                                                    />
                                                </div>
                                                {errors.contactPhone && <p className="text-rose-500 text-sm font-medium animate-in fade-in slide-in-from-left-2">{errors.contactPhone.message}</p>}
                                                <p className="text-slate-400 text-xs ml-1">Chúng tôi sẽ bảo mật số điện thoại này và chỉ dùng cho mục đích xác thực hồ sơ.</p>
                                            </div>
                                        </div>
                                    </motion.div>
                                )}
                                ...
                                {currentStep === 2 && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 border-l-4 border-indigo-500 pl-4 py-1">
                                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Xác thực danh tính</h2>
                                        </div>

                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div className="space-y-3">
                                                <Label className="text-slate-600 font-bold flex items-center gap-2">
                                                    CCCD Mặt trước <span className="text-rose-500">*</span>
                                                </Label>
                                                <div className={`relative border-2 border-dashed rounded-2xl transition-all duration-300 h-48 group overflow-hidden
                                                    ${watch('idCardFront') ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 hover:border-primary/40 hover:bg-slate-50/50'}`}
                                                >
                                                    {watch('idCardFront') ? (
                                                        <div className="flex flex-col items-center justify-center h-full gap-3">
                                                            ...
                                                            <p className="text-sm font-bold text-slate-700 truncate w-full text-center px-4">Đã chọn CCCD mặt trước</p>
                                                            <Button type="button" variant="ghost" size="sm" onClick={() => { setValue('idCardFront', ''); setFiles(p => ({ ...p, idCardFront: null })) }} className="h-8 text-rose-500 hover:text-rose-600 hover:bg-rose-50">Thay đổi</Button>
                                                        </div>
                                                    ) : (
                                                        <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer gap-3">
                                                            <UploadCloud className="w-10 h-10 text-slate-300 group-hover:text-primary/60 transition-colors" />
                                                            <p className="text-sm text-slate-500 text-center">Chụp hoặc chọn ảnh rõ nét</p>
                                                            <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, 'idCardFront')} />
                                                        </label>
                                                    )}
                                                </div>
                                                {errors.idCardFront && <p className="text-rose-500 text-xs font-semibold">{errors.idCardFront.message}</p>}
                                            </div>

                                            <div className="space-y-3">
                                                <Label className="text-slate-600 font-bold flex items-center gap-2">
                                                    CCCD Mặt sau <span className="text-rose-500">*</span>
                                                </Label>
                                                <div className={`relative border-2 border-dashed rounded-2xl transition-all duration-300 h-48 group overflow-hidden
                                                    ${watch('idCardBack') ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 hover:border-primary/40 hover:bg-slate-50/50'}`}
                                                >
                                                    {watch('idCardBack') ? (
                                                        <div className="flex flex-col items-center justify-center h-full gap-3">
                                                            <div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600">
                                                            ...
                                                            <p className="text-sm font-bold text-slate-700 truncate w-full text-center px-4">Đã chọn CCCD mặt sau</p>
                                                            <Button type="button" variant="ghost" size="sm" onClick={() => { setValue('idCardBack', ''); setFiles(p => ({ ...p, idCardBack: null })) }} className="h-8 text-rose-500 hover:text-rose-600 hover:bg-rose-50">Thay đổi</Button>
                                                        </div>
                                                    ) : (
                                                        <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer gap-3">
                                                            <UploadCloud className="w-10 h-10 text-slate-300 group-hover:text-primary/60 transition-colors" />
                                                ...
                                                {errors.idCardBack && <p className="text-rose-500 text-xs font-semibold">{errors.idCardBack.message}</p>}
                                            </div>
                                        </div>
                                    </motion.div>
                                )}
                                ...
                                {currentStep === 3 && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 border-l-4 border-amber-500 pl-4 py-1">
                                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Kinh nghiệm & Bằng cấp</h2>
                                        </div>

                                        <div className="space-y-4">
                                            <Label className="text-slate-600 font-bold flex items-center gap-2">
                                                CV / Resume chuyên môn <span className="text-rose-500">*</span>
                                            </Label>
                                            <div className={`p-6 border-2 rounded-2xl transition-all duration-300 flex items-center gap-5
                                                ${watch('cvUrl') ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-100 bg-slate-50/50 hover:bg-slate-50'}`}
                                            >
                                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center shadow-sm 
                                                    ${watch('cvUrl') ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-slate-300'}`}>
                                                    <FileText className="w-8 h-8" />
                                                </div>
                                                <div className="flex-1 overflow-hidden">
                                                    {watch('cvUrl') ? (
                                                        <p className="font-bold text-slate-700 truncate">{files.cvUrl?.name || "Tệp CV đã sẵn sàng"}</p>
                                                    ) : (
                                                        <div className="space-y-0.5">
                                                            <p className="font-bold text-slate-600">Thêm tệp PDF kinh nghiệm</p>
                                                            <p className="text-xs text-slate-400">Dung lượng tối đa 10MB</p>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="relative">
                                            ...
                                            {errors.cvUrl && <p className="text-rose-500 text-xs font-semibold ml-1">{errors.cvUrl.message}</p>}
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {errors.degreeUrls && <p className="text-rose-500 text-xs font-semibold ml-1">{errors.degreeUrls.message}</p>}
                                ...
                                {currentStep === 4 && (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 border-l-4 border-emerald-500 pl-4 py-1">
                                            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Kế hoạch khóa học & Cam kết</h2>
                                        </div>

                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                <Label className="text-slate-600 font-bold flex items-center gap-2">Mô tả tóm tắt nội dung bạn sẽ giảng dạy</Label>
                                                <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded-full uppercase tracking-tighter">Gợi ý 200 - 500 từ</span>
                                            </div>
                                            <Textarea
                                                ...
                                                rows={7}
                                                className="border-2 border-slate-100 focus:border-emerald-200 focus:bg-white bg-slate-50/50 rounded-2xl p-5 text-base leading-relaxed transition-all resize-none shadow-inner"
                                            />
                                        </div>

                                        <div className="space-y-6">
                                            <div className="bg-slate-900 rounded-[2rem] p-8 text-slate-300 shadow-2xl relative overflow-hidden">
                                            ...
                                            <div className="flex items-center space-x-3 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 group transition-all hover:border-primary/30">
                                                <Checkbox
                                                    id="terms"
                                                    checked={agreedTerms}
                                                ...
                                                />
                                                <label htmlFor="terms" className="text-sm sm:text-base font-extrabold text-slate-700 cursor-pointer select-none group-hover:text-primary transition-colors leading-tight">
                            ...
                            <div className="mt-12 flex items-center justify-between gap-4 pt-10 border-t border-slate-100">
                                {currentStep > 1 && (
                                    <Button
                                        ...
                                        className="h-14 px-8 rounded-2xl font-bold text-slate-600 border-2 hover:bg-slate-50 gap-2 min-w-[140px]"
                                    >
